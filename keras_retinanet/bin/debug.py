...
def run(generator, args, anchor_params):
    ...
    for i in range(generator.size()):
        ...
        image       = (np.clip((image + 1000) / 1400, 0., 1.) * 255).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    # DATA_DIR = '/mnt/data4/lty/victory_data/rescale_z_data_for_retinanet/'
    ...
    max_side = 768
    min_side = 768
    # '--random-transform', '--annotations', '--anchors', 
    main(['--random-transform', '--annotations', '--image-max-side', str(max_side), '--image-min-side', str(min_side), 'csv', str(train_anno_csv_path), str(classes_csv_path)])
